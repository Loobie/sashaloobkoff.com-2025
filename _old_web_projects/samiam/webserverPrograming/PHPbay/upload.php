...
<html>
  <head>
    <title>PHPbay Upload Confirmation</title>
  </head>
  <body>
  ...
  <?php
    ...
    if(isset($_FILES['photo']) && ($_POST['title']) && ($_POST['keywords']) && ($_POST['description']) ) {
      ...
      $query = "INSERT INTO auctionItems (url, title, keywords, description, Id) VALUES ('$url', '$title', '$keywords', '$description', '$Id')";
       
      $result = mysql_query($query)
         or die("Insert Failed.");
   
      // view item on php generated page
      print "-- Your auction item: <b>{$title}</b> has been posted. --";

      print "</table>";

    } else {
      print "Could not enter auction item into database.<br>Perhaps you haven't filled out all the fields.";
    }
  ?>
