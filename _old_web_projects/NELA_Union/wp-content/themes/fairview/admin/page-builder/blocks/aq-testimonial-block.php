...
if(!class_exists('AQ_Testimonial_Block')) {
	class AQ_Testimonial_Block extends AQ_Block {
		...
		function block($instance) {
			...
			<div id="testimonials_<?php echo rand(1,100) ?>" class="testimonials-block cf">
				<ul class="slides">
				...
				<?php foreach ($testimonials as $testimonial) {	
					...
					if(!empty($testimonial['author']) && !empty($testimonial['text'])) {
						...
						<li class="testimonial <?php echo $hide ?>">
							...
							<blockquote>
								<?php echo stripslashes($testimonial['text']) ?>
                                <cite>
                                    <h4 class="marB0"><strong><?php echo $author ?></strong></h4>
                                    <h5 class="marB0"><small><?php echo htmlspecialchars(stripslashes($testimonial['company'])) ?></small></h5>
                                </cite>
							</blockquote>
                            	<div class="clear"></div>
